...
const createMeetingBySalePerson = async (req , res , next)=>{
  try {
    ...
    if (!name || !email || !phone || !seduledDate || !start || !end || !organizationId || !videoCallLink || !meetingId) {
      return next(new ErrorHandler("Provide all neccessary field", 400))
    }
    ...
    const meeting = await MeetingModel.create({
      ...req.body,
      seduledDate: moment.tz(seduledDate, 'Asia/Kolkata').utc().format('YYYY-MM-DDTHH:mm:ss.SSS[Z]'),
      start: moment.tz(start, 'Asia/Kolkata').utc().format('YYYY-MM-DDTHH:mm:ss.SSS[Z]'),
      end: moment.tz(end, 'Asia/Kolkata').utc().format('YYYY-MM-DDTHH:mm:ss.SSS[Z]'),
  ...
  try {
    ...
    if(!meeting.salePersonId){
      ...
      if(isMeetingAvailable){
        return next(new ErrorHandler(`You have already a meeting on that time` , 400))
      }
    }
    ...
    if (req.body.seduledDate) {
      ...
      req.body.seduledDate = moment.tz(req.body.seduledDate, 'Asia/Kolkata').utc().format('YYYY-MM-DDTHH:mm:ss.SSS[Z]')
    }
    ...
    if (req.body.start) {
      ...
      req.body.start = moment.tz(req.body.start, 'Asia/Kolkata').utc().format('YYYY-MM-DDTHH:mm:ss.SSS[Z]')
    }
    ...
    if (req.body.end) {
      ...
      req.body.end = moment.tz(req.body.end, 'Asia/Kolkata').utc().format('YYYY-MM-DDTHH:mm:ss.SSS[Z]')
    }
